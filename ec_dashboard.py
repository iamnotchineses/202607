...
def load(file_bytes: bytes):
    ...
    _cat_map = {}                              # 세부 카테고리(A) → 그룹(B)
    _stock_rows = []                           # 재고: 브랜드/카테고리/수량/재고액
    try:
        wk = wb["카테고리분류"]
        for row in wk.iter_rows(min_row=2, values_only=True):
            ...
            if row[0] is not None and row[1] is not None:
                _cat_map[str(row[0]).strip()] = str(row[1]).strip()
            ...
            if ab and str(ab).strip() and len(row) >= 33:
                try:
                    ...
                    pass
            if len(row) >= 40 and row[19]:
                try:
                    _bnm = str(row[19]).strip()
                    if _bnm in ("PR", "사은품", "쇼핑백"):        # 재고 집계 제외
                        continue
                    _qty2 = float(row[32] or 0)                    # AG=가용수량
                    _avgc = float(row[38] or 0)                    # AM=원가평균
                    _stock_rows.append({
                        "브랜드": _bnm,
                        "카테고리": "리퍼" if _bnm == "리퍼" else str(row[22] or "").strip(),
                        "수량": _qty2,
                        "재고액": _avgc * _qty2,                    # 원가평균 × 가용수량
                    })
                except (TypeError, ValueError):
                    pass
    except Exception:
        pass
    try:
        _sdf = pd.DataFrame(_stock_rows)
        if len(_sdf):
            _sdf["카테고리"] = _sdf["카테고리"].map(lambda x: _cat_map.get(x, x))
            _sdf.loc[_sdf["카테고리"].astype(str).str.strip() == "", "카테고리"] = "(미분류)"
            stock_brand = (_sdf.groupby("브랜드")[["수량", "재고액"]].sum()
                           .sort_values("재고액", ascending=False))
            stock_cat = (_sdf.groupby("카테고리")[["수량", "재고액"]].sum()
                         .sort_values("재고액", ascending=False))
        else:
            stock_brand = pd.DataFrame(columns=["수량", "재고액"])
            stock_cat = pd.DataFrame(columns=["수량", "재고액"])
    except Exception:
        stock_brand = pd.DataFrame(columns=["수량", "재고액"])
        stock_cat = pd.DataFrame(columns=["수량", "재고액"])
    ...
    try:
        ...
        raw = pd.DataFrame(columns=["분류", "쇼핑몰", "브랜드", "상품", "수량", "매출", "이익",
                                    "날짜", "비고", "카테고리", "라인명", "필터일자"])
        pc = {"병행": {"매출": 0.0, "이익": 0.0, "수익율": 0.0, "수량": 0.0},
              "공식": {"매출": 0.0, "이익": 0.0, "수익율": 0.0, "수량": 0.0}}
...
if _dt_on:
    ...
    if not is_last:
        ...
        c[5].metric("예상마감", f"{cur_extra['예상']/1e8:,.2f}억", f"달성율 {_fc_ach:.1%}")
...
with c_left:
    ...
    st.dataframe(
        mdf.style.format({"매출": "{:,.0f}", "이익": "{:,.0f}", "객단가": "{:,.0f}",
                          "수익율": "{:.1%}",
                          "목표": "{:,.0f}", "달성율": "{:.1%}"}, na_rep="-"),
    ...
    if _dt_on:
        ...
        st.dataframe(
            pcdf.style.format({"매출": "{:,.0f}", "이익": "{:,.0f}", "객단가": "{:,.0f}",
                               "수익율": "{:.1%}"}, na_rep="-"),
            hide_index=True, use_container_width=True,
        )
    else:
        pcdf = pd.DataFrame([
            {"구분": lb, "목표": pc[lb]["목표"], "매출": pc[lb]["매출"], "이익": pc[lb]["이익"],
             "객단가": (pc[lb]["매출"] / pc[lb]["수량"]) if pc[lb].get("수량") else None,
             "수익율": pc[lb]["수익율"], "달성율": pc[lb]["달성율"]}
            for lb in ["병행", "공식"]
        ])
        st.dataframe(
...
with c_right:
    ...
    if _dt_on:
        ...
        _tot = pd.DataFrame([{"구분": "총합", "매출": _cs["매출"].sum(), "이익": _cs["이익"].sum(),
                              "객단가": (_cs["매출"].sum() / _tq) if _tq else None,
                              "수익율": (_cs["이익"].sum() / _cs["매출"].sum()) if _cs["매출"].sum() else 0}])
        _cs = pd.concat([_cs, _tot], ignore_index=True)
        _cs = _cs[["구분", "매출", "이익", "객단가", "수익율"]]
        st.dataframe(
            _cs.style.format({"매출": "{:,.0f}", "이익": "{:,.0f}", "객단가": "{:,.0f}",
                              "수익율": "{:.1%}"}, na_rep="-"),
...
if _q.strip():
    ...
    if _hit.empty:
        ...
        # 출고 상세 (개별 판매 건)
        _dt = _hit[["날짜", "쇼핑몰", "브랜드", "카테고리", "상품", "수량", "매출", "이익", "비고"]].copy()
        ...
        _dt = _dt[["날짜", "쇼핑몰", "브랜드", "카테고리", "상품", "수량", "판매단가", "매출", "수익", "수익율", "비고"]]
...
st.subheader("📦 재고 현황")
if len(stock_brand):
    _st_amt = float(stock_brand["재고액"].sum())
    _st_qty = float(stock_brand["수량"].sum())
    _stc1, _stc2 = st.columns(2)
    _stc1.metric("총 재고액 (원가)", f"{_st_amt/1e8:,.2f}억")
    _stc2.metric("총 재고수량 (가용)", f"{_st_qty:,.0f}개")
    _sb1, _sb2 = st.columns(2)
    with _sb1:
        st.markdown("**🏷 TOP10 브랜드 (재고액 순)**")
        _sb = stock_brand.head(10).reset_index()
        _sb["비중"] = _sb["재고액"] / _st_amt if _st_amt else 0.0
        _sb.insert(0, "#", range(1, len(_sb) + 1))
        st.dataframe(
            _sb.style.format({"수량": "{:,.0f}", "재고액": "{:,.0f}", "비중": "{:.1%}"}),
            hide_index=True, use_container_width=True,
            height=min(450, 60 + 35 * len(_sb)),
        )
    with _sb2:
        st.markdown("**🗂 카테고리별 (재고비중 vs 매출비중)**")
        _sc = stock_cat.reset_index()
        _sc["재고비중"] = _sc["재고액"] / _st_amt if _st_amt else 0.0
        _sales_cat = raw.groupby("카테고리")["매출"].sum()
        _sales_tot = float(_sales_cat.sum())
        _sc["매출비중"] = _sc["카테고리"].map(_sales_cat).fillna(0) / _sales_tot if _sales_tot else 0.0
        _sc["차이"] = _sc["매출비중"] - _sc["재고비중"]
        _sc = _sc[["카테고리", "수량", "재고액", "재고비중", "매출비중", "차이"]]

        def _diff_color(v):
            try:
                return "color:#c00" if float(v) < 0 else "color:#0a0"
            except (TypeError, ValueError):
                return ""
        _scs = _sc.style.format({"수량": "{:,.0f}", "재고액": "{:,.0f}", "재고비중": "{:.1%}",
                                 "매출비중": "{:.1%}", "차이": "{:+.1%}"})
        try:
            _scs = _scs.map(_diff_color, subset=["차이"])
        except AttributeError:
            _scs = _scs.applymap(_diff_color, subset=["차이"])
        st.dataframe(_scs, hide_index=True, use_container_width=True,
                     height=min(450, 60 + 35 * len(_sc)))
        st.caption("차이 = 매출비중 − 재고비중 (＋면 재고 대비 잘 팔림, −면 재고 과다)")
else:
    st.caption("재고 데이터 없음 (카테고리분류 시트 확인)")
